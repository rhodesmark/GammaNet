...
void stayAwake(int awakeCentiseconds) {
  ...
  delay(1000);
    ...
    while ((millis() - startMillis) < (awakeCentiseconds * 10)) {
        ...
        else{Serial.println(F("ERROR: -- No Node Type Selected --"));
        Serial.println(F("Core 1: Collecting GPS data..."));
        while (GPS_Serial.available() > 0) {
            if (gps.encode(GPS_Serial.read())) {
                if(debugMode == "On"){
        ...
        //End of If Else Case here depending on Node Type
...
void goToSleep(int sleepSeconds) {
                ...
                digitalWrite(Relay3v, LOW); // Turns relay ON
                }
                else{
                digitalWrite(Relay3v, LOW); // Turns relay ON
                } 
...
void getGPSInfo() {
    ...
    Serial.println(F("ALL GPS Data Collected Successfully"));
    //Node Type  Programme Will run below here.
}
