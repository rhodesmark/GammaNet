...
#include "systemSettings.h"  // Node Settings
#include "pinSetUp.h"  // Node Settings
#include "variables.h"  // Node Settings
#include "Sensors.h" //Sensores Heade File
...
void stayAwake(int centisecondsUntilNextSleep) {
    int durationSeconds = centisecondsUntilNextSleep / 100; // Convert centiseconds to seconds
    ...
    Serial.println(F("Core 1: Finished data collection, going to sleep..."));
    delay(100);
    goToSleep(300);
}
